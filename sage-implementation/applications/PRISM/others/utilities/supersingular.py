...
from utilities.pairing import weil_pairing_default
...
def generate_point_order_D(E, D, x_start=0, even_power=None):
    ...
    # If we know the even power, remove it from the cofactor
    if even_power:
        k >>= even_power
...
def compute_point_order_D(E, D, x_start=0, even_power=None):
    ...
    point of order D
    """
    return generate_point_order_D(
        E, D, x_start=x_start, even_power=even_power
    ).__next__()


def compute_linearly_independent_point_with_pairing(
    E, P, D, x_start=0, even_power=None
):
    """
    Input:  An elliptic curve E / Fp2
            A point P ∈ E[D]
    ...
    for Q in Qs:
        ...
        pair = weil_pairing_default(P, Q, D)
...
def compute_linearly_independent_point(E, P, D, x_start=0, even_power=None):
    ...
    """
    Q, _ = compute_linearly_independent_point_with_pairing(
        E, P, D, x_start=x_start, even_power=even_power
    )
    return Q
...
def torsion_basis_with_pairing(E, D, even_power=None):
    ...
    Q, ePQ = compute_linearly_independent_point_with_pairing(
        E, P, D, x_start=P[0], even_power=even_power
    )
...
# =========================================== #
#      Attempt at fast torsion for E[3^k]     #
# =========================================== #


def fast_3n_torsion_basis(E, ea, eb, f):
    """
    For a curve of order f*2^ea*3^eb compute
    the torsion basis of E[3^eb]

    Uses that two elements of order 3 are linearly
    dependent iff their x-coordinates are the same
    as for P in E[3] then we have 3P = O or
    P + [2]P = O so x(P) and x([2]P) are the same


    """
    Ps = generate_kummer_point(E)
    for R in Ps:
        # Obtain a point in the [3^eb]-torsion
        P = clear_cofactor(R, f, even_power=ea)

        # Get the point of order 3
        P3 = P.triple_iter(eb - 1)

        # If P3 is not zero, P has order 3^eb
        if P3.is_zero():
            continue
    ...
    xP = P.x()
    Qs = generate_kummer_point(E, x_start=xP)
    for R in Qs:
        # Obtain a point in the [3^eb]-torsion
        Q = clear_cofactor(R, f, even_power=ea)

        # Get the point of order 3
        Q3 = Q.triple_iter(eb - 1)

        # If Q3 is zero, the order is wrong
        if Q3.is_zero():
            continue

        # If the x-coordinate matches, the points are linerly dependent
        if Q3.x() == xP:
            continue

        return P.curve_point(), Q.curve_point()
...
def compute_canonical_kernel(imP, imQ, D, basis=None, ePQ=None):
    ...
    if basis == None:
        ...
        P, Q = basis
        if ePQ is None:
            ePQ = weil_pairing_default(P, Q, D)
