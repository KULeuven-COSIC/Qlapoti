...
from utilities.pairing import weil_pairing_default, tate_pairing_default, weil_pairing_power_two, tate_pairing_power_two
...
def BiDLP(R, P, Q, D, ePQ=None):
    ...
    if ePQ:
        ...
        pair_PQ = weil_pairing_default(P, Q, D)

    # Write R = aP + bQ for unknown a,b
    # e(R, Q) = e(P, Q)^a
    pair_a = weil_pairing_default(R, Q, D)

    # e(R,-P) = e(P, Q)^b
    pair_b = weil_pairing_default(R, -P, D)
...
def BiDLP_power_two(R, P, Q, e, window = None, ePQ=None, tate = False):
    r"""
    ...
    if tate:
        if tate == True:
            ...
            exp = (p**2 - 1) // 2**e
        else: # assume this is an integer giving the exponentiation
            exp = tate
    ...
    if ePQ:
        ...
        if tate:
            pair_PQ = tate_pairing_power_two(P, Q, e, exp=exp)
        else:
            pair_PQ = weil_pairing_power_two(P, Q, e)

    # Write R = aP + bQ for unknown a,b
    if tate:
        # e(R, Q) = e(P, Q)^a
        pair_a = tate_pairing_power_two(Q, -R, e, exp=exp)

        # e(R,-P) = e(P, Q)^b
        pair_b = tate_pairing_power_two(P, R, e, exp=exp)
    else:
        pair_a = weil_pairing_power_two(R, Q, e)
        pair_b = weil_pairing_power_two(R, -P, e)
...
def multi_BiDLP_power_two(Rs, P, Q, e, window = None, ePQ=None, tate = False):
    if ePQ is None:
        if tate:
            if tate == True:
                p = P.curve().base_ring().characteristic()
                exp = (p**2 - 1) // 2**e
                ePQ = tate_pairing_power_two(P, Q, e, exp=exp)
            else: # assume this is an integer giving the exponentiation
                exp = tate
        else:
            ePQ = weil_pairing_power_two(P, Q, e)
    return (BiDLP_power_two(R, P, Q, e, window=window, ePQ=ePQ, tate=tate) for R in Rs)
...
def DLP_power_two(R, P, Q, e, window, ePQ=None, first=True):
    ...
    if ePQ:
        ...
        pair_PQ = tate_pairing_power_two(P, Q, D, exp=exp)

    if first:
        pair_a = tate_pairing_power_two(Q, -R, D, exp=exp)
        x = windowed_pohlig_hellman(pair_a, pair_PQ, e, window)

    else:
        pair_b = tate_pairing_power_two(P, R, D, exp=exp)
